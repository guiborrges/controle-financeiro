(function initMobileV2Mes(global) {
  ...
  function renderPlanejamento(month) {
    ...
    return `<div class="m2-tab-panel ${activeSubtab === 'planejamento' ? 'active' : ''}" data-tab-panel="planejamento">${renderListCard('Compromissos do mês', rows)}</div>`;
  }
    ...
    return `
      <div class="m2-tab-panel ${activeSubtab === 'renda' ? 'active' : ''}" data-tab-panel="renda">
        <section class="m-list-card">
          ...
          `).join('') : '<div class="m2-empty">Sem rendas cadastradas no mês.</div>'}
