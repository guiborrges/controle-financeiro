...
async function loadLoginProfile() {
  try {
    const developerSessionResponse = await fetch('/api/developer/session', {
      credentials: 'same-origin',
      headers: { Accept: 'application/json' }
    });
    if (developerSessionResponse.ok) {
...
function openDeveloperModal() {
  ...
  const error = document.getElementById('developerError');
  const password = document.getElementById('developerPassword');
  if (error) error.textContent = '';
  if (modal) modal.hidden = false;
  window.setTimeout(() => password?.focus(), 20);
}
...
async function submitDeveloperLogin(event) {
  event.preventDefault();
  const form = event.currentTarget;
  const passwordInput = document.getElementById('developerPassword');
  const errorEl = document.getElementById('developerError');
  const submitButton = form.querySelector('button[type="submit"]');
  ...
  try {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      errorEl.textContent = payload.message || 'Nao foi possivel autenticar a area tecnica.';
      passwordInput.focus();
      passwordInput.select();
      return;
    }
    ...
    errorEl.textContent = 'Falha ao conectar com a area do desenvolvedor.';
  } finally {
    submitButton.disabled = false;
...
document.getElementById('developerForm').addEventListener('submit', submitDeveloperLogin);
...
document.getElementById('developerClose').addEventListener('click', closeDeveloperModal);
...
document.getElementById('toggleDeveloperPassword').addEventListener('click', () => {
  const input = document.getElementById('developerPassword');
  const button = document.getElementById('toggleDeveloperPassword');
  const showing = input.type === 'text';
  input.type = showing ? 'password' : 'text';
  button.setAttribute('aria-label', showing ? 'Mostrar senha do desenvolvedor' : 'Ocultar senha do desenvolvedor');
  button.textContent = showing ? 'Mostrar' : 'Ocultar';
});
...
document.addEventListener('keydown', event => {
  ...
  if (event.key === 'Escape') {
    if (!document.getElementById('registerModal')?.hidden) closeRegisterModal();
