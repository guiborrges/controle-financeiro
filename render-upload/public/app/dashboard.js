...
function getDashboardResultMode() {
  return isGenericDashboardUser() ? 'simples' : resultMode;
}
...
function getDashboardResultLabel(filtered) {
  return isGenericDashboardUser() ? 'Resultado do mês' : labelResult(filtered);
}
...
function renderDashboard() {
  ...
  const filtered = getFilteredData().map(m => { ensureProjetos(m); return m; });
  ...
  if (isPrimaryUserEnvironment()) buildResultSelect('mesResultSelect', filtered);
  renderTitles();
  renderDashSeriesControls();
  ...
  document.getElementById('dashSub').textContent = `${filtered.length} meses no periodo - ${first?.nome || ''}${first && last && first !== last ? ' - ' + last.nome : ''}`;
  ...
  const metricCards = {
    resultado: `
      <div class="metric-card ${(isGenericDashboardUser() ? resCur : resTotal) >= 0 ? 'green' : 'red'}" draggable="true" data-metric-key="resultado" ondragstart="onDashMetricDragStart(event,'resultado')" ondragend="onDashMetricDragEnd(event)" ondragover="onDashMetricDragOver(event)" ondragleave="onDashMetricDragLeave(event)" ondrop="onDashMetricDrop(event,'resultado')">
        <div class="mc-label">${isGenericDashboardUser() ? 'Resultado do mês' : `${getDashboardResultLabel(filtered)} - periodo`}</div>
        <div class="mc-value">${fmtSigned(isGenericDashboardUser() ? resCur : resTotal)}</div>
      </div>
    `,
    gastos: `
      <div class="metric-card red" draggable="true" data-metric-key="gastos" ondragstart="onDashMetricDragStart(event,'gastos')" ondragend="onDashMetricDragEnd(event)" ondragover="onDashMetricDragOver(event)" ondragleave="onDashMetricDragLeave(event)" ondrop="onDashMetricDrop(event,'gastos')">
        <div class="mc-label">${isGenericDashboardUser() ? 'Gastos total' : 'Total acumulado gastos'}</div>
    ...
    ganhos: `
      <div class="metric-card green" draggable="true" data-metric-key="ganhos" ondragstart="onDashMetricDragStart(event,'ganhos')" ondragend="onDashMetricDragEnd(event)" ondragover="onDashMetricDragOver(event)" ondragleave="onDashMetricDragLeave(event)" ondrop="onDashMetricDrop(event,'ganhos')">
        <div class="mc-label">${isGenericDashboardUser() ? 'Ganhos total' : 'Total acumulado ganhos'}</div>
    ...
    renda: `
      <div class="metric-card green" draggable="true" data-metric-key="renda" ondragstart="onDashMetricDragStart(event,'renda')" ondragend="onDashMetricDragEnd(event)" ondragover="onDashMetricDragOver(event)" ondragleave="onDashMetricDragLeave(event)" ondrop="onDashMetricDrop(event,'renda')">
        <div class="mc-label">${sectionTitles.renda} acumulada</div>
    ...
    projetos: `
      <div class="metric-card" style="background:var(--blue-light);border-color:rgba(40,85,160,.2)" draggable="true" data-metric-key="projetos" ondragstart="onDashMetricDragStart(event,'projetos')" ondragend="onDashMetricDragEnd(event)" ondragover="onDashMetricDragOver(event)" ondragleave="onDashMetricDragLeave(event)" ondrop="onDashMetricDrop(event,'projetos')">
        <div class="mc-label">${sectionTitles.projetos} acumulados</div>
    ...
    result: `
      <div class="dashboard-widget" data-widget-key="result" style="--widget-x:${dashboardWidgetLayout.result.x}px;--widget-y:${dashboardWidgetLayout.result.y}px;--widget-w:${dashboardWidgetLayout.result.w}px;--widget-h:${dashboardWidgetLayout.result.h}px">
        <div class="section">
          <div class="section-head">
            <div class="dashboard-widget-head">
              <h3 id="chartTitleRes">Resultado por mes</h3>
  ...
  charts['lineChart'] = new Chart(lc, {
    ...
    options: { responsive:true, maintainAspectRatio:false, plugins:{legend:{display:true, position:'bottom', labels:{boxWidth:12, boxHeight:12, padding:14, font:{size:11}},
      ...
      scales:{ x:{ticks:{font:{size:11}}}, y:{ticks:{callback:v=>'R$'+(v/1000).toFixed(0)+'k'},grid:{color:'rgba(0,0,0,.05)'}} } }
  });
  ...
  const cats = getCatTotalsPeriod(filtered);
        ...
        <div class="cat-grid" style="grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));align-content:start;min-width:0;gap:12px">
          ${catKeys.map(k => `
            <div class="cat-item" ondblclick="openCategoryColorPicker('${String(k).replace(/'/g, "\\'")}', event.clientX + 12, event.clientY + 12)" style="min-width:0;padding:12px 14px;align-items:center;background:rgba(255,255,255,.75);border:1px solid rgba(140,118,96,.12);border-radius:14px;cursor:pointer">
              <div class="cat-dot" style="background:${getCategoryColor(k)}"></div>
      ...
      charts['catPieChart'] = new Chart(pieCanvas.getContext('2d'), {
        ...
        data: {
          ...
          datasets: [{
            ...
            backgroundColor: catKeys.map(k => getCategoryColor(k)),
            borderColor: '#fffaf4',
            borderWidth: 3,
            hoverOffset: 8
          }]
